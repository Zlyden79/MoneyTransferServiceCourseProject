...
* ParametersValidationException - выбрасывается сервисом, если провалилась валидация параметров.
* ExternalServiceException - если бизнес-логика выдаёт отказ в операции.
...
* Для интеграционного теста необходимо сбилдить образ командой ```docker build -t backend . ```

## Журналирование

За журналирование отвечает класс MoneyTransferServiceLogger. В журнал пишут методы сервисов и обработчик исключений.
